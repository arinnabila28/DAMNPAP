...
class PeminjamanController extends BaseController
{
    ...
    // 2. AMAN: Cek apakah ID bukunya valid sebelum dicari
        $id_buku = $peminjaman['id_book'] ?? null;
         $buku = ($id_buku) ? $bookModel->find($id_buku) : null;

    // 3. Hitung Denda
        $tgl_deadline = strtotime($peminjaman['tanggal_kembali']);
        $tgl_sekarang = strtotime(date('Y-m-d'));
        $denda = ($tgl_sekarang > $tgl_deadline) ? (floor(($tgl_sekarang - $tgl_deadline) / 86400) * 1000) : 0;

    // 4. Update stok HANYA jika data buku ditemukan
    if ($buku) {
        $bookModel->update($id_buku, ['stock' => ($buku['stock'] + 1)]);
    }

    // 5. Selesaikan transaksi
    $model->delete($id, true);

    session()->setFlashdata('success', 'Buku dikembalikan. Denda: Rp ' . number_format($denda));
    return redirect()->to('/list/peminjaman');
    }
}